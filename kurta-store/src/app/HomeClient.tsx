...
import type { Product, DesignConfig, LookbookHotspotData, SizeLabel } from '@/types/schema';

const CATEGORIES = ['All', 'Casual', 'Festive', 'Wedding', 'Work'] as const;
type Category = (typeof CATEGORIES)[number];

const MOCK_PRODUCTS: Product[] = [
  { id: 'prod-indigo', title: 'Indigo Linen Block-Print Kurta', slug: 'prod-indigo', description: 'Handwoven indigo linen, straight cut.', priceINR: 1299, images: ['/prod-bestseller.webp'], sizes: { XS: 3, S: 8, M: 12, L: 5, XL: 2, XXL: 0 }, category: 'Casual', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-ivory', title: 'Classic Ivory Cotton Kurta', slug: 'prod-ivory', description: 'Soft-washed cotton, A-line silhouette.', priceINR: 1499, images: ['/prod-ivory.webp'], sizes: { XS: 2, S: 5, M: 8, L: 6, XL: 3, XXL: 1 }, category: 'Casual', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-rose', title: 'Rose Chanderi Anarkali', slug: 'prod-rose', description: 'Sheer rose pink chanderi, floor length.', priceINR: 3499, images: ['/prod-anarkali.webp'], sizes: { XS: 2, S: 5, M: 7, L: 3, XL: 1, XXL: 0 }, category: 'Festive', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-navy', title: 'Navy Chikankari Straight Kurta', slug: 'prod-navy', description: 'Navy cotton with chikankari embroidery.', priceINR: 1899, images: ['/prod-navy.webp'], sizes: { XS: 0, S: 3, M: 6, L: 4, XL: 2, XXL: 0 }, category: 'Work', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-blush', title: 'Blush Tissue Gharara Set', slug: 'prod-blush', description: 'Bridal tissue silk with tilla embroidery.', priceINR: 4999, images: ['/prod-wedding.webp'], sizes: { XS: 1, S: 2, M: 3, L: 2, XL: 1, XXL: 0 }, category: 'Wedding', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-terra', title: 'Terracotta Festive Kurta', slug: 'prod-terra', description: 'Mulberry silk with gold border details.', priceINR: 2299, images: ['/prod-festive.webp'], sizes: { XS: 0, S: 4, M: 6, L: 4, XL: 2, XXL: 1 }, category: 'Festive', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-pastel', title: 'Minaara Pastels Kurta Set', slug: 'prod-pastel', description: 'Pastel pink and mint with hand-done detailing.', priceINR: 2499, images: ['/new-arrivals.webp'], sizes: { XS: 1, S: 4, M: 6, L: 4, XL: 2, XXL: 0 }, category: 'Casual', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
  { id: 'prod-ochre', title: 'Ochre Cotton Mul Kurta', slug: 'prod-ochre', description: 'Breathable mul cotton with delicate pintucks.', priceINR: 1699, images: ['/prod-festive.webp'], sizes: { XS: 0, S: 3, M: 5, L: 4, XL: 2, XXL: 0 }, category: 'Casual', isActive: true, isFeatured: true, collectionId: 'mock-coll', variants: [], createdAt: '2025-01-01', updatedAt: '2025-01-01' },
];
...
export default function HomeClient({
  ...
  const products = initialProducts.length ? initialProducts : MOCK_PRODUCTS;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ctx = gsap.context(() => {
        ...
        // 3. Featured pieces horizontal scroll (pinned)
        ...
        // 4. About Us horizontal scroll (pinned)
        ...
        // 5. Universal reveals — declared after pinned sections so offsets are correct
        ...
        // 6. Editorial cards clip-path unroll
        ...
        // 10. Marquee words 3D tilt on scroll
  ...
  return (
    <main style={{ position: 'relative', overflow: 'hidden' }}>
      <NarrativeLoader isLoading={false} />
      ...
      <div style={{ overflowX: 'hidden' }}>
        ...
        <section ref={heroRef} style={{ position: 'relative', height: '100vh', marginTop: '-64px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          <Image src="/hero-woman.webp" alt="SS 2025 — Dressed in Grace" fill className="object-cover object-center" priority sizes="100vw" style={{ opacity: 0.85 }} />
          ...
          <div className="hero-content" style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '820px', margin: '0 auto', padding: '0 48px' }}>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
              ...
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
                ...
                Dressed in{' '}<em style={{ color: '#C4AC70', fontStyle: 'italic' }}>Grace</em>
              </motion.h1>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.6 }} style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'center' }}>
                <Link href="#collection" id="hero-explore-cta"
                  style={{ padding: '14px 40px', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.18em', backgroundColor: '#32518C', color: '#C4AC70', borderRadius: '4px', textDecoration: 'none', fontWeight: 600, border: '1px solid rgba(196,172,112,0.35)' }}>Explore Collection</Link>
                <Link href="/lookbook"
                  style={{ padding: '14px 40px', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.18em', backgroundColor: 'rgba(26,26,26,0.08)', border: '1px solid rgba(26,26,26,0.25)', color: '#1A1A1A', borderRadius: '4px', textDecoration: 'none' }}>View Lookbook</Link>
              </motion.div>
            </motion.div>
          </div>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 48px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '40px', gap: '24px', flexWrap: 'wrap' }}>
              <div>
                ...
                <h2 className="scroll-heading" style={{ fontSize: 'clamp(2rem,3vw,2.75rem)', fontWeight: 300, color: '#1A1A1A', margin: 0 }}>The Collection</h2>
              </div>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {CATEGORIES.map((cat) => (
                  <button key={cat} id={`filter-${cat.toLowerCase()}`} onClick={() => setActiveCategory(cat)}
                    style={{ padding: '6px 20px', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.12em', borderRadius: '100px', border: `1px solid ${activeCategory === cat ? '#8C6F63' : '#E6E2D8'}`, backgroundColor: activeCategory === cat ? '#8C6F63' : 'transparent', color: activeCategory === cat ? '#fff' : 'rgba(26,26,26,0.6)', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <AnimatePresence mode="wait">
              {filtered.length === 0 ? (
                <div key="empty" style={{ padding: '80px 0', textAlign: 'center', color: 'rgba(26,26,26,0.35)' }}>No pieces in this category yet.</div>
              ) : (
                  ...
                  {filtered.map((product, i) => (
                    <motion.div key={product.id} initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06, duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
                      <ProductCard product={product} fmt={fmt}
                        onAdd={(size) => { const variantId = product.variants.find((v) => v.size === size)?.id || `mock-variant-${product.id}-${size}`; addItem({ productId: product.id, variantId, title: product.title, size, quantity: 1, priceINR: product.priceINR, imageUrl: product.images[0] ?? '' }); trackAddToCart(product, size, 1); }} />
                    </motion.div>
                  ))}
                </motion.div>
              )}
        ...
        <div ref={hFeatRef} style={{ position: 'relative', backgroundColor: '#0A0A0A', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', top: '48px', left: '48px', zIndex: 20 }}>
            <p className="fade-up" style={{ fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.25em', color: 'rgba(244,236,225,0.45)', marginBottom: '8px' }}>✦ Drag to explore</p>
            <h2 className="scroll-heading" style={{ fontSize: 'clamp(2rem,3vw,2.8rem)', fontWeight: 300, color: '#fff', margin: 0 }}>Featured Pieces</h2>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', height: '100vh', padding: '0 48px', paddingTop: '140px', position: 'relative', zIndex: 10 }}>
                ...
                <div key={`hx-${product.id}`} style={{ width: '300px', flexShrink: 0 }}>
                  <Link href={`/product/${product.id}`} style={{ textDecoration: 'none', display: 'block' }}>
                    <div className="hx-card" style={{ position: 'relative', height: '420px', borderRadius: '12px', overflow: 'hidden', backgroundColor: '#1A1A1A', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                      <Image src={product.images[0]} alt={product.title} fill className="object-cover hx-img" sizes="300px" style={{ transition: 'transform 0.8s var(--ease-out-expo)' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', columnGap: '24px', rowGap: '84px' }}>
              {NEW_ARRIVALS.map((product, i) => (
                <motion.div key={`na-${product.id}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ delay: i * 0.08, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}>
                  <ProductCard product={product} fmt={fmt} onAdd={(size) => { const variantId = product.variants.find((v) => v.size === size)?.id || `mock-variant-${product.id}-${size}`; addItem({ productId: product.id, variantId, title: product.title, size, quantity: 1, priceINR: product.priceINR, imageUrl: product.images[0] ?? '' }); trackAddToCart(product, size, 1); }} />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div ref={hAboutRef} style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #F4ECE1 0%, #FAF8F5 50%, #EDE6DE 100%)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', height: '100vh', paddingTop: '140px', paddingBottom: '80px', position: 'relative', zIndex: 10 }}>
            <div ref={hAboutTrackRef} style={{ display: 'flex', gap: '32px', padding: '0 48px', willChange: 'transform' }}>
              {ABOUT_PANELS.map((panel, i) => (
                <div key={i} style={{ width: 'calc(100vw - 120px)', maxWidth: '640px', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', alignItems: 'center', backgroundColor: 'rgba(250, 248, 245, 0.95)', border: '1px solid rgba(255, 255, 255, 0.65)', borderRadius: '20px', padding: '32px', boxShadow: '0 12px 40px rgba(15, 42, 91, 0.04)' }}>
                    <div>
              ...
              {[
                ...
                <div key={i} className="edit-card" style={{ position: 'relative', aspectRatio: '3/4', borderRadius: '12px', overflow: 'hidden', cursor: 'pointer' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: '28px', left: '28px', right: '28px', color: '#fff' }}>
                    ...
                    <Link href="/lookbook" style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#fff', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.38)', paddingBottom: '2px' }}>Shop Look →</Link>
                  </div>
                </div>
              ))}
            </div>
        ...
        <footer style={{ backgroundColor: 'rgba(23, 57, 115, 0.95)', borderTop: '1px solid rgba(196, 172, 112, 0.2)', padding: '48px 0' }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '24px' }}>
            <Link href="/" style={{ fontSize: '1.2rem', color: '#F4ECE1', opacity: 0.75, fontStyle: 'italic', textDecoration: 'none' }}>Minaara Creation</Link>
            ...
            <p style={{ fontSize: '11px', color: 'rgba(244,236,225,0.3)', margin: 0 }}>© {new Date().getFullYear()} Minaara Creation</p>
          </div>
        </footer>
